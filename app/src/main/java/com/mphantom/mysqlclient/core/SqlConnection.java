...
public class SqlConnection implements Runnable {
    private String TAG = SqlConnection.class.getName();
...
//    InputStream inputStream;
    ...
    public SqlConnection(ConnectionInfo info) {
        ...
        try {
            ...
            channel.connect(new InetSocketAddress(info.getHost(), info.getPort()));

            new Thread(this).start();
        } catch (IOException e) {
            e.printStackTrace();
            Log.d(TAG, "the error in the socket connection");
        }
    }
...
//                    Log.d(TAG, HexUtil.Bytes2HexString(buffer.get));
